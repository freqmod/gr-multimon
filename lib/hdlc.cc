...
/* ---------------------------------------------------------------------- */
multimon_hdlc_sptr
make_multimon_hdlc()//(const std::string &args)
{
  //return multimon_afsk1200_sptr(new multimon_afsk1200());
  return gnuradio::get_initial_sptr(new multimon_hdlc());
}
...
void hdlc_rxbit(struct demod_state *s, int bit){ //dummy to please headers
}
void hdlc_rxbit_ext(struct demod_state *s, const unsigned char bit, unsigned char** packet, int* pkglen)
{
	...
	if ((s->l2.hdlc.rxbitstream & 0xff) == 0x7e) {
		if (s->l2.hdlc.rxstate && (s->l2.hdlc.rxptr - s->l2.hdlc.rxbuf) > 2){
            ...
            *pkglen = s->l2.hdlc.rxptr - s->l2.hdlc.rxbuf;
            *packet = s->l2.hdlc.rxbuf;
        }
		s->l2.hdlc.rxstate = 1;
...
int 
multimon_hdlc::general_work (int noutput_items,
...
//  afsk12_demod(state, (const float*) input_items[0], ninput_items[0], (unsigned char*) output_items[0], &outlen);
  ...
  for (int i = 0; i < ninput_items[0]; i++){
    hdlc_rxbit_ext(s, in[i], &packet, &pkglen);
//    printf("Sentbit[%d]%d\n", i, in[i]);
    if(packet!=NULL){
//        packet[pkglen]=0;
        if (pkglen > 10){    	   	
